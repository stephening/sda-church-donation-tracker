...
namespace Donations.Model
{
	...
	public class AdventistGiving
	{
		...
		/// <param name="tx"></param>
		/// <returns></returns>
		static public string AGHash(AdventistGiving tx)
		{
			return String.Concat(tx.LastName, tx.FirstName, tx.Address, tx.Zip);
		}

		/// <summary>
		/// This static function will import from an Adventist giving csv and return an
		...
		/// </exception>
		static public ObservableCollection<AdventistGiving>? Import(string path)
		{
			...
			using (var reader = di.FileSystem.File.OpenText(path))
			{
				...
				while (!string.IsNullOrEmpty(line = reader.ReadLine()))
				{
					...
					if (split != null && split.Length == headers.Length)
					{
						...
						record.DonorHash = AGHash(record);
