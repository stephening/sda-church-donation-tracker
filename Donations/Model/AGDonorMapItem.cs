...
namespace Donations.Model
{
	...
	public class AGDonorMapItem
	{
		...
		/// </summary>
		[JsonIgnore]
		public string? LastName => (null != di.Data.DonorDict && di.Data.DonorDict.ContainsKey(DonorId)) ? di.Data.DonorDict[DonorId].LastName : "";
		/// The FirstName property is not stored in the database, and is obtained from a lookup
		/// by DonorId in the local databse. The property is provided for visual confirmation
		/// in the table provided for editing AG to local donor mapping.
		[JsonIgnore]
		public string? FirstName => (null != di.Data.DonorDict && di.Data.DonorDict.ContainsKey(DonorId)) ? di.Data.DonorDict[DonorId].FirstName : "";
		/// The Address property is not stored in the database, and is obtained from a lookup
		/// by DonorId in the local databse. The property is provided for visual confirmation
		/// in the table provided for editing AG to local donor mapping.
		[JsonIgnore]
		public string? Address => (null != di.Data.DonorDict && di.Data.DonorDict.ContainsKey(DonorId)) ? di.Data.DonorDict[DonorId].Address : "";
		/// The City property is not stored in the database, and is obtained from a lookup
		/// by DonorId in the local databse. The property is provided for visual confirmation
		/// in the table provided for editing AG to local donor mapping.
		[JsonIgnore]
		public string? City => (null != di.Data.DonorDict && di.Data.DonorDict.ContainsKey(DonorId)) ? di.Data.DonorDict[DonorId].City : "";
		/// The State property is not stored in the database, and is obtained from a lookup
		/// by DonorId in the local databse. The property is provided for visual confirmation
		/// in the table provided for editing AG to local donor mapping.
		[JsonIgnore]
		public string? State => (null != di.Data.DonorDict && di.Data.DonorDict.ContainsKey(DonorId)) ? di.Data.DonorDict[DonorId].State : "";
		/// The Zip property is not stored in the database, and is obtained from a lookup
		/// by DonorId in the local databse. The property is provided for visual confirmation
		/// in the table provided for editing AG to local donor mapping.
		[JsonIgnore]
		public string? Zip => (null != di.Data.DonorDict && di.Data.DonorDict.ContainsKey(DonorId)) ? di.Data.DonorDict[DonorId].Zip : "";
		...
		/// <summary>
		/// This Copy function is supplied to make copies of the r/w properties of the mapping object, so that
		/// in the edit maintenance Tabs, edits are kept local until saved.
		/// </summary>
		/// <returns>Copy of AGDonorMapItem</returns>
		public AGDonorMapItem Copy()
		{
			return new AGDonorMapItem()
			{
				AGLastName = AGLastName,
				AGFirstName = AGFirstName,
				AGAddress = AGAddress,
				AGCity = AGCity,
				AGDonorHash = AGDonorHash,
				AGState = AGState,
				AGZip = AGZip,
				DonorId = DonorId,
			};
