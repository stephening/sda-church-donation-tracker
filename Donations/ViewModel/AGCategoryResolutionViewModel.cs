...
namespace Donations.ViewModel
{
	...
	/// </summary>
	public class AGCategoryResolutionViewModel : INotifyPropertyChanged
	{
		...
		/// </summary>
		public Visibility CategoryDiffsVisibility
		{
			get { return _categoryDiffsVisibility; }
			set
			{
				_categoryDiffsVisibility = value;
				OnPropertyChanged();
			}
		}
		...
		/// </summary>
		public Visibility CategoryResolutionComplete
		{
			get { return _categoryResolutionComplete; }
			set
			{
				_categoryResolutionComplete = value;
				OnPropertyChanged();
			}
		}

		/// <summary>
		/// The ContinueCategoryResolutionCmd property is the Command binding for the 'Continue...' button.
		/// </summary>
		public IAsyncRelayCommand ContinueCategoryResolutionCmd { get; }
		...
		{
			di.CategoryResolution = this;

			// Setup the command handlers for the button Command bindings
			ContinueCategoryResolutionCmd = new AsyncRelayCommand(ContinueCategoryResolution);
		}
		...
		private void CategoryResolutionLoop(int start)
		{
			for (_txIdx = start; _txIdx < TransactionList?.Count; _txIdx++)
			{
				...
				if (true == di.Data.AGCategoryMap?.ContainsKey(categoryId))
				{
					// exact match through map
					Transaction = tx;
					Cat = di.Data.CatDict[di.Data.AGCategoryMap[categoryId].CategoryCode];
					continue;
				}
				...
				foreach (var item in di.Data.CatDict)
				{
		...
		public async Task ContinueCategoryResolution()
		{
			await Task.Run(() =>
			{
				...
				if (null != Transaction && null != di.Data.AGCategoryMap && true != di.Data.AGCategoryMap?.ContainsKey(Transaction.CategoryCode))
				{
					// add to dictionary if not there already
					di.Data.AGCategoryMap[Transaction.CategoryCode] = new AGCategoryMapItem()
					{
						AGCategoryCode = Transaction.CategoryCode,
						AGCategoryName = Transaction.CategoryName,
						CategoryCode = Cat.Code,
					};
					di.Data.AGCategoryMapList?.Add(di.Data.AGCategoryMap[Transaction.CategoryCode]);
				}
