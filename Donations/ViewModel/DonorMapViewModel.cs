...
namespace Donations.ViewModel
{
	...
	/// </summary>
	public partial class DonorMapViewModel : INotifyPropertyChanged
	{
		...
		/// any changes which have been made in the DonorMapView tab.
		/// </summary>
		...
		/// all donors that don't match exactly will have to be mapped again.
		/// </summary>
		...
		public DonorMapViewModel()
		{
			...
			foreach (var entry in di.Data.AGDonorMapList)
			{
				DonorMapList.Add(entry.Copy());
			}
		...
		public void DeleteRow()
		{
			...
			{
				HasChanges = true;
				DonorMapList.RemoveAt(SelectedRowIndex.Value);
			}
		}
		...
		{
			di.Data.AGDonorMap.Clear();
			di.Data.AGDonorMapList.Clear();

			foreach (var entry in DonorMapList)
			{
				di.Data.AGDonorMap[entry.AGDonorHash] = entry.Copy();
				di.Data.AGDonorMapList.Add(di.Data.AGDonorMap[entry.AGDonorHash]);
			}
